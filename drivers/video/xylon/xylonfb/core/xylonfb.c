	...
	struct xylonfb_layer_data *layer_data)
{
	...
	if ((common_data->display_interface_type >> 4) == LOGICVC_DI_ITU656) {
		YKr  = 29900;
		YKg  = 58700;
		YKb  = 11400;
		YK   = 1600000;
	...
	struct fb_info *fbi)
{
	...
	common_data->reg_access.xylonfb_set_reg_val(var->yoffset,
		...
		layer_data);
	common_data->reg_access.xylonfb_set_reg_val((var->xres-1),
		layer_data->layer_reg_base_virt, LOGICVC_LAYER_HOR_POS_ROFF,
		layer_data);
	/* apply changes in logiCVC */
	common_data->reg_access.xylonfb_set_reg_val((var->yres-1),
...
static void xylonfb_set_yvirt(struct xylonfb_layer_fix_data *lfdata,
	...
	unsigned long vmem_base_addr, u32 reg_base_phys, void *reg_base_virt)
{
	...
	if (layer_data->xylonfb_cd->xylonfb_flags & FB_DMA_BUFFER) {
		/* NOT USED FOR NOW! */
		layer_data->fb_virt = dma_alloc_coherent(dev,
...
static void xylonfb_set_hw_specifics(struct fb_info *fbi,
	struct xylonfb_layer_data *layer_data, struct xylonfb_layer_fix_data *lfdata,
	u32 reg_base_phys)
{
	driver_devel("%s\n", __func__);
...
int xylonfb_init_driver(struct xylonfb_init_data *init_data)
{
	...
	common_data->display_interface_type = init_data->display_interface_type;
